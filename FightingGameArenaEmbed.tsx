...
const FIGHTING_GAME_ASSET_VERSION = 'engine-20260623-hitstop03';
...
export function FightingGameArenaEmbed({
  ...
  const leftAvatar = leftSide?.logoUrl || leftSide?.avatarUrl || arenaAgentAvatar(leftName);
  const rightAvatar = rightSide?.logoUrl || rightSide?.avatarUrl || arenaAgentAvatar(rightName);
  ...
  const fighterPaths = useMemo(() => {
    const resolveFighterJson = (avatarUrl: string) => {
      if (avatarUrl?.includes('icon-5.jpg') || avatarUrl?.toLowerCase().includes('pepe')) {
        return `/2dgame/data/fighters/pepe.json?v=${FIGHTING_GAME_ASSET_VERSION}`;
      }
      if (avatarUrl?.toLowerCase().includes('floki')) {
        return `/2dgame/data/fighters/floki.json?v=${FIGHTING_GAME_ASSET_VERSION}`;
      }
      if (avatarUrl?.includes('icon-1.png') || avatarUrl?.toLowerCase().includes('boxban') || avatarUrl?.toLowerCase().includes('kinsmen')) {
        return `/2dgame/data/fighters/boxban.json?v=${FIGHTING_GAME_ASSET_VERSION}`;
      }
      if (avatarUrl?.includes('icon-4.jpg') || avatarUrl?.toLowerCase().includes('pengu')) {
        return `/2dgame/data/fighters/pengu.json?v=${FIGHTING_GAME_ASSET_VERSION}`;
      }
      if (avatarUrl?.includes('icon-2.png') || avatarUrl?.toLowerCase().includes('shiba')) {
        return `/2dgame/data/fighters/shiba.json?v=${FIGHTING_GAME_ASSET_VERSION}`;
      }
      return `/2dgame/data/fighters/bantah-mascot-green.json?v=${FIGHTING_GAME_ASSET_VERSION}`;
    };

    return [
      resolveFighterJson(leftAvatar),
      resolveFighterJson(rightAvatar),
    ];
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      if (!window.__botaArenaMusicElement) {
        try {
          const audio = new Audio('/2dgame/audio/bota-arena-loop.mp3');
          audio.loop = true;
          audio.volume = 0.3;
          window.__botaArenaMusicElement = audio;
        } catch (e) {}
      }
