;(function () {
  ...
  mk.controllers.Base.prototype._initializeFighters = function (fighters) {
    ...
    this._opponents[0] = this.fighters[1];
    this._opponents[1] = this.fighters[0];
    this.fighters[0]._index = 0;
    this.fighters[1]._index = 1;
  };

  mk.controllers.Base.prototype.getOpponent = function (f) {
    return this._opponents[f._index];
  };
  ...
  mk.controllers.Base.prototype._attackCompatible = function (attack, opponentStand) {
    ...
    if (opponentStand === m.WIN || opponentStand === m.FALL) {
      return false;
    }
  ...
  mk.controllers.AI = function (options) {
    mk.controllers.Base.call(this, options);
  };

  mk.controllers.AI.prototype = new mk.controllers.Base();

  mk.controllers.AI.prototype._initialize = function () {
    var self = this;
    self._aiIntervals = [];
    self.fighters.forEach(function (fighter, idx) {
      var interval = setInterval(function () {
        self._tickAI(fighter, idx);
      }, 300 + idx * 50);
      self._aiIntervals.push(interval);
    });
  };

  mk.controllers.AI.prototype._tickAI = function (fighter, idx) {
    if (this._gameOver) return;

    var m = mk.moves.types,
      opponent = this.fighters[idx === 0 ? 1 : 0],
      currentMove = fighter.getMove().type,
      fMid = fighter.getX() + fighter.getWidth() / 2,
      oMid = opponent.getX() + opponent.getWidth() / 2,
      distance = Math.abs(fMid - oMid),
      width = fighter.getWidth(),
      attacks = [m.HIGH_PUNCH, m.LOW_PUNCH, m.HIGH_KICK, m.LOW_KICK, m.UPPERCUT, m.SPIN_KICK],
      rand = Math.random();

    if (currentMove === m.WIN || currentMove === m.FALL ||
        currentMove === m.KNOCK_DOWN || currentMove === m.ENDURE ||
        currentMove === m.SQUAT_ENDURE || currentMove === m.ATTRACTIVE_STAND_UP) {
      return;
    }

    if (currentMove === m.SQUAT) {
      fighter.setMove(m.STAND_UP);
      return;
    }

    if (distance <= width) {
      if (rand < 0.08) {
        fighter.setMove(m.BLOCK);
      } else {
        var attack = attacks[Math.floor(Math.random() * attacks.length)];
        fighter.setMove(attack);
      }
    } else if (distance <= width * 2.5 && rand < 0.15) {
      fighter.setMove(m.FORWARD_JUMP);
    } else {
      if (fMid < oMid) {
        fighter.setMove(m.WALK);
      } else {
        fighter.setMove(m.WALK_BACKWARD);
      }
    }
  };

  mk.controllers.AI.prototype.reset = function () {
    if (this._aiIntervals) {
      this._aiIntervals.forEach(function (id) { clearInterval(id); });
      this._aiIntervals = [];
    }
    this._gameOver = false;
  };

  mk.controllers.AI.prototype._restartRound = function () {
    var self = this,
      m = mk.moves.types,
      conf = mk.config;
    this._gameOver = false;
  ...
  mk.start = function (options) {
    ...
    switch (type) {
      ...
      case 'webcaminput':
        ...
        break;
      case 'ai':
        mk.game = new mk.controllers.AI(options);
        break;
      default:
        mk.game = new mk.controllers.Basic(options);
    }
  ...
  mk.arenas.Arena.prototype.init = function () {
    var conf = mk.config;
    var bgEl = document.getElementById('arena-bg');
    if (bgEl) {
      bgEl.style.backgroundImage = 'url(' + conf.IMAGES + conf.ARENAS + this.arena + '/arena.png)';
    }
  ...
  mk.arenas.Arena.prototype.refresh = function () {
    this._drawArena();
    var f, img, scale = mk.config.FIGHTER_SCALE;
    for (var i = 0; i < this.fighters.length; i += 1) {
      f = this.fighters[i];
      img = f.getState();
  ...
  mk.arenas.Arena.prototype.moveFighter = function (fighter, pos) {
    ...
    var leftBound = mk.config.ARENA_LEFT_BOUND;
    var rightBound = this.width - mk.config.ARENA_RIGHT_BOUND - fighter.getVisibleWidth();
    if (pos.x <= leftBound) {
      pos.x = leftBound;
    }
    if (pos.x >= rightBound) {
